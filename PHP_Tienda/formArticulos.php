...
<body>
	...
	<form action="formArticulos.php" action="POST">
		...
		<p><input type="hidden" name="id" value="<?php echo $datosProducto["ProductID"]; ?>"></p>

		<p><label>Nombre:: </label> <input type="text" name="nombre" value="<?php echo $datosProducto["Name"]; ?>">
		</p>
		<p><label>Coste: </label> <input type="number" name="coste" value="<?php echo $datosProducto["Cost"]; ?>">
		</p>
		<p><label>Precio: </label> <input type="number" name="precio" value="<?php echo $datosProducto["Price"]; ?>">
		</p>
		<p><label>Categoria: </label> <select name="categoria">
				<?php echo pintaCategorias($datosProducto["CategoryID"]); ?>
			</select>
		</p>
		...
		if (isset($_GET["Editar"])) {
			...
			echo "<input type='submit' name='Accion' value='Anadir'>";
		}
			...
			switch ($_GET['Accion']) {
				...
				case "Anadir":
					if (anadirProducto($_GET["nombre"], $_GET["coste"], $_GET["precio"], $_GET["categoria"])) {
						...
						echo "Ha ocurrido un error al agregar el nuevo producto.";
					}
					break;
			}
		}
